...
```bash
$ node -r dotenv/config your_script.js dotenv_config_path=/custom/path/to/.env.local dotenv_config_debug=true
```
...
```bash
$ DOTENV_CONFIG_ENCODING=latin1 DOTENV_CONFIG_DEBUG=true node -r dotenv/config your_script.js dotenv_config_path=/custom/path/to/.env.local
```
...
[dotenvx@0.14.1] injecting env (2) from .env.local
DATABASE_URL postgres://username@localhost/my_database
```
...
```sh
$ dotenvx run --debug -- node index.js
[dotenvx@0.14.1] injecting env (1) from .env.local
...
```bash
$ echo "HELLO=production" > .env.local.production
$ echo "console.log('Hello ' + process.env.HELLO)" > index.js

$ dotenvx run -f=.env.local.production -- node index.js
Hello production
> ^^
```
...
```bash
$ echo "HELLO=local" > .env.local.local
$ echo "HELLO=World" > .env.local
$ echo "console.log('Hello ' + process.env.HELLO)" > index.js

$ dotenvx run -f=.env.local.local -f=.env.local -- node index.js
Hello local
```
...
```sh
$ dotenvx encrypt -f .env.local.production
```
...
```sh
$ node index.js
[dotenv@17.0.0] injecting env (1) from .env.local
Hello World
```
